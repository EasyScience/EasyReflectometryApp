...
APPLE_CERT_DATA = sys.argv[
    3
]  # Encoded content of the .p12 certificate file (exported from certificate of Developer ID Application type)
...
APPLE_NOTARY_PASSWORD = sys.argv[6]  # App specific password for EasyReflectometry from https://appleid.apple.com
...
def signMacos():
    try:
        ...
        try:
            ...
            Functions.run('security', 'create-keychain', '-p', keychain_password, keychain_name)
        ...
        try:
            ...
            Functions.run('security', 'default-keychain', '-s', keychain_name)
        except Exception as sub_exception:
            Functions.printFailMessage(sub_message, sub_exception)
            sys.exit(1)
        else:
        ...
        try:
            ...
            Functions.run('security', 'list-keychains')
        except Exception as sub_exception:
            Functions.printFailMessage(sub_message, sub_exception)
            sys.exit(1)
        else:
        ...
        try:
            ...
            Functions.run('security', 'unlock-keychain', '-p', keychain_password, keychain_name)
        except Exception as sub_exception:
            Functions.printFailMessage(sub_message, sub_exception)
            sys.exit(1)
        else:
        ...
        try:
            ...
            Functions.run(
                ...
                '-T',
                '/usr/bin/codesign',
            )  # Without '-T ...' codesign asking to enter keychain password and thus CI freezes
        except Exception as sub_exception:
            Functions.printFailMessage(sub_message, sub_exception)
            sys.exit(1)
        else:
        ...
        try:
            ...
            Functions.run('security', 'find-identity', keychain_name)
        except Exception as sub_exception:
            Functions.printFailMessage(sub_message, sub_exception)
            sys.exit(1)
        else:
        ...
        try:
            ...
            Functions.run(
                ...
                '--keychain',
                keychain_name,  # specify keychain name
                '--identifier',
                CONFIG['ci']['codesign']['apple']['product_id'],  # specify bundle id
                '--sign',
                CONFIG['ci']['codesign']['apple']['team_id'],  # sign the code at the path(s) given using this identity
        ...
        try:
            ...
            Functions.run('codesign', '--display', '--verbose', CONFIG.setup_exe_path)
        except Exception as sub_exception:
            Functions.printFailMessage(sub_message, sub_exception)
            sys.exit(1)
        else:
        ...
        try:
            ...
            Functions.run('ditto', '-c', '-k', '--rsrc', '--sequesterRsrc', CONFIG.setup_exe_path, CONFIG.setup_zip_path_short)
        except Exception as sub_exception:
            Functions.printFailMessage(sub_message, sub_exception)
            sys.exit(1)
        else:
        ...
        try:
            ...
            Functions.run(
                ...
                APPLE_NOTARY_USER,
                '--team-id',
                CONFIG['ci']['codesign']['apple']['team_id'],
                '--password',
        ...
        try:
            ...
            Functions.run('spctl', '--assess', '--verbose', CONFIG.setup_exe_path)
        except Exception as sub_exception:
            Functions.printFailMessage(sub_message, sub_exception)
            sys.exit(1)
        else:
