...
#        self._project_info = json.loads(json_str)
    ...
    def editProjectInfo(self, key, value):
        if key == 'location':
            self._project.current_path = value
...
#        return self._currentProjectPath
        return str(self._project.current_path)

    @currentProjectPath.setter
    def currentProjectPath(self, new_path):
        if self._project.current_path == new_path:
            return
        self._project.current_path = new_path
...
#        self._saveProject()
    ...
    def projectFilePath(self):
        return self._project.path_project_json
